 ...
 * @LastEditTime: 2023-05-26 11:34:36
...
<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(order, index) in orderInfo.detailArrayList" :key="skuId">
          <li>
      ...
      <div class="bbs">
        <h5>买家留言：</h5>
        <textarea placeholder="建议留言前先与商家沟通确认" class="remarks-cont" v-model="customerMsg"></textarea>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{userDeliveryAddress[0].fullAddress}}</span>
        收货人：<span>{{userDeliveryAddress[0].consignee}}</span>
        <span>{{userDeliveryAddress[0].phoneNum}}</span>
      </div>
    </div>
...
import { mapState } from "vuex";
  export default {
    ...
    computed: {
         ...
         */
        userDeliveryAddress(){
            // 这里可以用filter（返回数组）或find（返回对象）
            return this.userAddressList.filter((item) => {
